...
r = permutation_importance(model, df_test[X], df_test[y], n_repeats=30, random_state=0)
...
if model_name == "svm":
    ax = sns.scatterplot(x="sepal_length", y="petal_length", hue=y, data=df_train, s=20)
...
sns.move_legend(strp, loc="lower right", title="Added Variables")
if model_name != "svm":
    plt.xlim(-0.05, 1)
plt.tight_layout()
...
# %%
absolute_df = df.copy()
absolute_df[x] = np.abs(absolute_df[x])

ax = sns.barplot(
    x="Variable",
    y=x,
    data=absolute_df.query("`Added Variables` == 10"),
)
...
ax = sns.barplot(
    ...
    data=df.query("`Added Variables` == 10"),
)
plt.xticks(rotation=90)
...
shap.plots.bar(
    shap_values, clustering=clust, clustering_cutoff=0.9, show=False, max_display=12
)
fig.subplots_adjust(left=0.20, right=0.9)
fig.savefig(fig_out / f"{model_name}_shap_clustbar_12.png")
# %%
fig, ax = plt.subplots(1, 1, figsize=(12, 6))
shap.plots.bar(
    shap_values, clustering=clust, clustering_cutoff=0.9, show=False, max_display=2
)
fig.subplots_adjust(left=0.40, right=0.9)
fig.savefig(fig_out / f"{model_name}_shap_clustbar_2.png")
# %%
